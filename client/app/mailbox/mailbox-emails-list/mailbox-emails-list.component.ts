  ...
  ngOnInit() {
    this.paramsSub = this.route.params.subscribe(params => {
      if (params['mailbox'] == null) {
        this.seoService.updateMetaTag({ name: 'description', content: 'ooTech DTES - mailboxes'});
        this.seoService.setTitle('ooTech DTES - Mailboxes');
      } else {
      }
      this.emailId = params['emailId'];
      if (!this.mailbox || this.mailbox.toLowerCase() !== params['mailbox'].toLowerCase()) {
        this.mailbox = params['mailbox'].toLowerCase();
        this.seoService.updateMetaTag({ name: 'description', content: 'ooTech DTES - ' + this.mailbox});
        this.seoService.setTitle('ooTech DTES - ' + this.mailbox);
