<p align="center">
  <img width="460" src="https://github.com/o4oren/Ad-Hoc-Email-Server/blob/master/client/assets/svg/dtes-happy-top-bottom-dk.svg?raw=true">
</p>
...
<h2 align="center">
  Ad Hoc Email Server (DTES)
</h2>
Ad Hoc Email Server is, well, an ad hoc mail server. 
DTES can be used for testing, where a large (or unknown) number of mailboxes are needed or to provide disposable emails for registering for services that might expose the email to spam;
This project will provide both a RESTful API and web application for consuming the service.

## Getting Started
DTES will accept any email sent to it (on the domains specified in the configuration), and create an ad hoc mailbox as needed.
...
### Configuration
DTES use several environment variables for configuration. These can be set up using a .env file (see [dotEnv docuemntation](https://www.npmjs.com/package/dotenv)).
...
mongoConnectUrl | The mongodb connect URL in the form of "mongodb://localhost:27017/dtes".
...
### Docker
* Build docker: docker build -t o4oren/dtes .
* Build FE only: docker build -t o4oren/dtes-universal . -f Dockerfile_ssr
* Run docker with external properties: docker run -v /[local.dotEvn.path]/.env:/opt/dtes/.env -it -p 3000:3000 -p 25:25 -d o4oren/dtes
* Sign into docker: docker exec -it dtes sh
* start docker stackand swarm: docker swarm init && docker stack deploy -c docker-compose.yml dtesswarmc
...
#### Palette ####
The DTES color palette is available [here](https://material.io/tools/color/#!/?view.left=0&view.right=0&primary.color=515f77&secondary.color=E3F2FD).

### API
The full API documentation is available [here](https://www.temp.ootech.vn/help/api "DTES - API Help").
...
DTES server is ready for Google Analytics. By default, when you set it up, it will not send information to GA though.
...
See also the list of [contributors](https://github.com/o4oren/dtes-server/contributors) who participated in this project.
