...
describe('Emails', () => {
  ...
  test('List emails', done => {
    request(server)
      ...
      .end((err, res) => {
        ...
        expect(emailInfo.subject).toBe('DTES mail test! ✔');
        done();
      });
  });
  ...
  test('Get attachment', done => {
    request(server)
      .get('/api/mailbox/alive-test/email/' + emailInfo.emailId + '/attachments/dtes-happy.png')
      .set('Content-type', 'application/json')
      .expect(200)
  ...
  test('Get attachment that doesn\'t exist', done => {
    request(server)
      .get('/api/mailbox/alive-test/email/' + emailInfo.emailId + '/attachments/dtes-happppppy.png')
      .set('Content-type', 'application/json')
      .expect(404, done);
  });
